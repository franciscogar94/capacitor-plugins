  ...
  implements CapacitorGoogleMapsPlugin
{
  ...
  async handlerDirections(args: {
    ...
    map.DirectionRendered = new google.maps.DirectionsRenderer();
    map.DirectionRendered.setMap(map.map);
    map.DirectionRendered.setOptions({});
    map.DirectionRendered.setDirections(args.result);
    return;
  }

  /**Remove all polylines generated by the method Directions*/
  async removeAllDirectionsPolylines(args: { id: string }): Promise<void> {
    const map = this.maps[args.id];
    map.DirectionRendered?.setMap(null);
    map.DirectionRendered = undefined;
    return;
  }
  ...
  async removeAllMarkers(_args: { id: string }): Promise<void> {
    const map = this.maps[_args.id];
